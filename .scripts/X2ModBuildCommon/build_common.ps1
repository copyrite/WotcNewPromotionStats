...
# TODO: verify that delete-excluded is the default behaviour I want (I believe so currently, but not 100% certain)
# not sure that --prune-empty-dirs is a good idea, but until it causes problems I am keeping it
$global:def_rsync_args = @("-r", "--delete-excluded", "--prune-empty-dirs")
...
class BuildProject {
	...
	[void]InvokeBuild() {
		try {
			...
			# Windows sounds not supported on linux
			throw
		}
	}
	...
	[void]_SetupUtils() {
		...
		if (Test-Path $modSrcPath) {
			# This seems like a very odd difference between windows and linux powershell, but we also need to split the path, or we get the full path which does not work for our purposes
			$this.modScriptPackages = @(Split-Path -Path (Get-ChildItem "$($this.modSrcRoot)/Src" -Directory) -Leaf)
		} else {
	...
	[void]_CopyModToSdk() {
		$xf = @("--exclude=*.x2proj", "--exclude=/ContentForCook")

		if (![string]::IsNullOrEmpty($this.contentOptionsJsonFilename)) {
			$xf += "--exclude=$($this.contentOptionsJsonFilename)"
		}
		...
		# Powershell does not care about which way the slashes point; rsync does, so we make sure everything is correct
		# The cleaner way would be to replace the around 150 or so backwardsslashes in this script that are not used inside a pattern but I am lazy
		$src_path_for_rsync = Convert-Path $this.modSrcRoot
		if(-not (Test-Path $this.stagingPath))
		{
			New-Item $this.stagingPath -ItemType Directory
		}
		$dest_path_for_rsync = Convert-Path $this.stagingPath
		# Needed correction to make rsync copy the stuff into the folder the dest folder instead of creating another folder in there.
		# alternatively could adjust staging path but that might break other things in the script and I do not care to find out.
		$src_path_for_rsync = "$src_path_for_rsync/"
		$dest_path_for_rsync = "$dest_path_for_rsync/"
		rsync $global:def_rsync_args @xf "$src_path_for_rsync" "$dest_path_for_rsync"
	...
	[void]_CopyToSrc() {
		...
		# Powershell does not care about which way the slashes point; rsync does, so we make sure everything is correct
		# The cleaner way would be to replace the around 150 or so backwardsslashes in this script that are not used inside a pattern but I am lazy
		$src_path_for_rsync = Convert-Path "$($this.sdkPath)\Development\SrcOrig"
		# if the dest path does not exist Convert-Path errors out and from googling there is no way to prevent this, so we make sure the pathe exists
		if(-not (Test-Path $this.devSrcRoot))
		{
			New-Item $this.devSrcRoot -ItemType Directory
		}
		$dest_path_for_rsync = Convert-Path $this.devSrcRoot
		# Needed correction to make rsync copy the stuff into the folder the dest folder instead of creating another folder in there.
		# alternatively could adjust staging path but that might break other things in the script and I do not care to find out.
		$src_path_for_rsync = "$src_path_for_rsync/"
		$dest_path_for_rsync = "$dest_path_for_rsync/"
		# First include is needed for recursive copy
		rsync $global:def_rsync_args "--include=*/" "--include=*.uc" "--include=*.uci" "--exclude=*" "$src_path_for_rsync" "$dest_path_for_rsync"
	...
	[void]_RunCookHL() {
		...
		# TODO: VERIFY THIS IS CORRECT IN PARTICULAR IF STUFF GETS COPIED INTO THE RIGHT PLACE

		# Powershell does not care about which way the slashes point; rsync does, so we make sure everything is correct
		# The cleaner way would be to replace the around 150 or so backwardsslashes in this script that are not used inside a pattern but I am lazy
		$src_path_for_rsync = Convert-Path $cookedpcconsoledir
		if(-not (Test-Path $this.cookerOutputPath))
		{
			New-Item $this.cookerOutputPath -ItemType Directory
		}
		$dest_path_for_rsync = Convert-Path $this.cookerOutputPath
		# Needed correction to make rsync copy the stuff into the folder the dest folder instead of creating another folder in there.
		# alternatively could adjust staging path but that might break other things in the script and I do not care to find out.
		$src_path_for_rsync = "$src_path_for_rsync/"
		$dest_path_for_rsync = "$dest_path_for_rsync/"
		# "--include=*/" should not be needed because we do not recurse (I think)
		rsync "--include=*.tfc" "--exclude=*" "--ignore-existing" "$src_path_for_rsync" "$dest_path_for_rsync"
	...
	[void]_FinalCopy() {
		...
		# Powershell does not care about which way the slashes point; rsync does, so we make sure everything is correct
		# The cleaner way would be to replace the around 150 or so backwardsslashes in this script that are not used inside a pattern but I am lazy
		$src_path_for_rsync = Convert-Path $this.stagingPath
		if(-not (Test-Path $this.finalModPath))
		{
			New-Item $this.finalModPath -ItemType Directory
		}
		$dest_path_for_rsync = Convert-Path $this.finalModPath
		# Needed correction to make rsync copy the stuff into the folder the dest folder instead of creating another folder in there.
		# alternatively could adjust staging path but that might break other things in the script and I do not care to find out.
		$src_path_for_rsync = "$src_path_for_rsync/"
		$dest_path_for_rsync = "$dest_path_for_rsync/"
		rsync $global:def_rsync_args "$src_path_for_rsync" "$dest_path_for_rsync"
	}
	...
	[void]_InvokeEditorCmdlet([StdoutReceiver] $receiver, [string] $makeFlags, [int] $sleepMsDuration) {
		...
		# TODO: Find out what and how to modify this to start the process in wine
		# This seems to work just fine
		# possibly make this use proton; the only benefit of this would be not having to juggle two prefixes
		$Wine_Arguments = @("$($this.commandletHostPath)", $makeFlags)
		$pinfo = New-Object System.Diagnostics.ProcessStartInfo
		$pinfo.FileName = "wine"
		$pinfo.RedirectStandardOutput = $true
		$pinfo.RedirectStandardError = $true
		$pinfo.UseShellExecute = $false
		$pinfo.Arguments = $Wine_Arguments
		# The path below CAN NOT contain spaces i.e. 
		# YOU HAVE TO CREATE A SYMLINK TO THE SDK THAT DOES NOT CONTAIN SPACES IN ITS PATH AND USE THAT AS THE SDK LOCATION IN THE USER SETTINGS JSON.
